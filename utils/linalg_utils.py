...
def pdist2(X: torch.Tensor,
    ...
    if order == PDist2Order.d_second:
        ...
        if Z is None:
            Z = X
            G = X.transpose(-2, -1) @ Z
            R = X.pow(2).sum(-2, keepdim=True)
            S = R.transpose(-2, -1)
        else:
            if Z.dim() == 2:
                Z = Z.unsqueeze(-1)
            G = X.transpose(-2, -1) @ Z
            S = X.pow(2).sum(-2, keepdim=True).transpose(-2, -1)
            R = Z.pow(2).sum(-2, keepdim=True)

    return torch.sqrt(torch.abs(R + S - 2 * G)).squeeze(1).squeeze(-1)
